...
class FirebaseClient:
    ...
    def __init__(self):
        ...
        # Properly handle absolute vs relative paths
        if not os.path.isabs(service_account_path):
            # Only for relative paths, make it relative to project root
    ...
    def log_collection_run(self, collection_stats: Dict[str, Any]) -> str:
        ...
        try:
            ...
            traceback.print_exc()
            return ""
    def get_keywords(self, max_retries: int = 3, retry_delay: float = 2.0) -> List[str]:
        """Get active keywords from Firebase youtube_keywords collection with retry logic"""
        import time
        import random
        
        for attempt in range(1, max_retries + 1):
            try:
                self.logger.info(f"Retrieving keywords from Firebase (attempt {attempt}/{max_retries})...")
                ...
                self.logger.info(f"Successfully retrieved {len(keywords)} active keywords from {doc_count} documents")
                self.logger.info(f"Keywords retrieved at: {current_time}")
                self.logger.info(f"Active keywords: {keywords}")
    ...
    def update_keyword_collection_timestamp(self, keyword: str, timestamp: datetime = None) -> bool:
        """Update the last_interval_collection timestamp for a keyword"""
        try:
            if timestamp is None:
                timestamp = datetime.utcnow()
            
            # Get keyword document reference
            keyword_ref = self.db.collection('youtube_keywords').document(keyword)
            
            # Check if document exists
            keyword_doc = keyword_ref.get()
            if not keyword_doc.exists:
                self.logger.warning(f"Keyword document '{keyword}' does not exist, cannot update timestamp")
                return False
            
            # Update the last_interval_collection field
            keyword_ref.update({
                'last_interval_collection': timestamp
            })
            
            self.logger.info(f"Updated last_interval_collection for keyword '{keyword}' at {timestamp.isoformat()}")
            return True
            
        except Exception as e:
            self.logger.error(f"Failed to update last_interval_collection for keyword '{keyword}': {e}")
            return False
